...
Nothing in this file touches the network. Every client the store builds is
answered by a handler defined a few lines above the assertion, and `data_dir`
is always a tmp_path, so a real cache on the machine running the tests cannot
leak into them.
"""
...
async def test_an_empty_token_does_not_call_an_endpoint_that_must_refuse_it(
    ...
    assert await store(tmp_path, satnogs_db_token="").refresh(KNACKSAT2) == tel.NO_TOKEN


async def test_an_empty_token_is_reported_rather_than_drawn_as_a_broken_panel(
        tmp_path, monkeypatch):
    """The panel has to be able to say which variable to set. "No data" sends
    an operator to the logs for something that is one line of .env."""
    no_network(monkeypatch)
    ...
    assert snap["status"] == tel.NO_TOKEN
...
async def test_a_rejected_token_does_not_become_a_retry_loop(tmp_path, monkeypatch):
    ...
    seen = answer_with(monkeypatch, lambda r: httpx.Response(401, json={"detail": "no"}))
    s = store(tmp_path)

    assert await s.refresh(KNACKSAT2) == tel.BAD_TOKEN
    assert await s.refresh(KNACKSAT2) == tel.BAD_TOKEN
    assert len(seen) == 1, "the second poll went back to the API anyway"
...
async def test_an_empty_result_set_is_not_an_error(tmp_path, monkeypatch):
    """A satellite nobody has decoded this week is a fact, not a failure."""
    answer_with(monkeypatch, ok(page()))
    s = store(tmp_path)
...
async def test_an_unreachable_db_keeps_the_frames_it_already_had(
        tmp_path, monkeypatch):
...
async def test_a_page_of_junk_is_not_read_as_no_frames(tmp_path, monkeypatch):
    ...
    history to a captive portal's login page."""
    answer_with(monkeypatch, ok(page(frame())))
    s = store(tmp_path)
    await s.refresh(KNACKSAT2)

    answer_with(monkeypatch, ok({"results": "not a list"}))
    assert await s.refresh(KNACKSAT2, force=True) == tel.UNREACHABLE
    assert len(s.get(KNACKSAT2)) == 1
...
async def test_frames_survive_a_restart_with_no_token_at_all(tmp_path, monkeypatch):
    ...
    assert await after.refresh(KNACKSAT2) == tel.NO_TOKEN
...
async def test_the_endpoint_answers_a_shaped_200_when_there_is_no_token(
        tmp_path, monkeypatch):
    """Not a 4xx, not a 503: an unconfigured token is a state to render, and a
    fetch error would leave the rest of the panel with nothing to say."""
    no_network(monkeypatch)
    s = store(tmp_path, satnogs_db_token="")
    ...
    assert body["status"] == tel.NO_TOKEN
    assert body["frames"] == []
    json.dumps(body)
